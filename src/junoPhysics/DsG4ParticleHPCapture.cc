...
//
#include "DsG4ParticleHPCapture.hh"
#include "G4ParticleHPManager.hh"
#include "G4SystemOfUnits.hh"
#include "DsG410GdParticleHPCaptureFS.h"
#include "DsG410NNDCParticleHPCaptureFS.h"
...
#include "G4Threading.hh"
  ...
  DsG4ParticleHPCapture::DsG4ParticleHPCapture()
...
//    G4cout <<"G4ParticleHPChannel constructed"<<G4endl;
    DsG4ParticleHPCaptureFS * theFS = new DsG4ParticleHPCaptureFS;
...
//  //    G4cout << "initializing theCapture "<<i<<" "<< numEle<<G4endl;
    ...
    delete theFS;
//    G4cout << "-------------------------------------------------"<<G4endl;
//    G4cout << "Leaving DsG4ParticleHPCapture::DsG4ParticleHPCapture"<<G4endl;
*/
  }
  ...
  G4HadFinalState * DsG4ParticleHPCapture::ApplyYourself(const G4HadProjectile& aTrack, G4Nucleus& aNucleus )
  {
    ...
    if(getenv("NeutronHPCapture")) G4cout <<" ####### DsG4ParticleHPCapture called"<<G4endl;
...
const std::pair<G4double, G4double> DsG4ParticleHPCapture::GetFatalEnergyCheckLevels() const
{
...
/*
void DsG4ParticleHPCapture::addChannelForNewElement()
{
   DsG4ParticleHPCaptureFS* theFS = new DsG4ParticleHPCaptureFS;
   for ( G4int i = numEle ; i < (G4int)G4Element::GetNumberOfElements() ; i++ ) 
   {
      G4cout << "DsG4ParticleHPCapture Prepairing Data for the new element of " << (*(G4Element::GetElementTable()))[i]->GetName() << G4endl;
      theCapture.push_back( new G4ParticleHPChannel );
      (*theCapture[i]).Init((*(G4Element::GetElementTable()))[i], dirName);
      (*theCapture[i]).Register(theFS);
   }
...
G4int DsG4ParticleHPCapture::GetVerboseLevel() const
{
   return G4ParticleHPManager::GetInstance()->GetVerboseLevel();
}
void DsG4ParticleHPCapture::SetVerboseLevel( G4int newValue ) 
{
   G4ParticleHPManager::GetInstance()->SetVerboseLevel(newValue);
}

void DsG4ParticleHPCapture::BuildPhysicsTable(const G4ParticleDefinition&)
{
   ...
   if ( G4Threading::IsMasterThread() ) {
      ...
      for ( G4int i = numEle ; i < (G4int)G4Element::GetNumberOfElements() ; i++ ) 
      {
         ...
         theCapture->push_back( new G4ParticleHPChannel );
         if(elemZ == 64) { // Gd.
           DsG410GdParticleHPCaptureFS * theGdFS = new DsG410GdParticleHPCaptureFS;
           ((*theCapture)[i])->Init((*(G4Element::GetElementTable()))[i], dirName);
           ...
           delete theGdFS;
                 ...
                 || elemZ == 26 /*Fe*/
                 || elemZ == 28 /*Ni*/) {
           DsG410NNDCParticleHPCaptureFS * theNNDCFS = new DsG410NNDCParticleHPCaptureFS();
           ((*theCapture)[i])->Init((*(G4Element::GetElementTable()))[i], dirName);
...
void DsG4ParticleHPCapture::ModelDescription(std::ostream& outFile) const
{
