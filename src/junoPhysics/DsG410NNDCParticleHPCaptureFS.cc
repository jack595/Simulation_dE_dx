...
//                            
// Modified class from original DsG4GdParticleHPCaptureFS class
//-------------------------------------------------------------------------
...
DsG410NNDCParticleHPCaptureFS::DsG410NNDCParticleHPCaptureFS() 
{
    hasXsec = false; 
    targetMass = 0;
}
...
G4HadFinalState * DsG410NNDCParticleHPCaptureFS::ApplyYourself(const G4HadProjectile & theTrack)
{
    G4int i;
    ...
    theTwo->SetDefinition(G4IonTable::GetIonTable()->GetIon(static_cast<G4int>(theBaseZ), static_cast<G4int>(theBaseA+1), 0));
    ...
    if (1==nPhotons) {
        ...
        G4ParticleDefinition * aRecoil = G4IonTable::GetIonTable()->GetIon(static_cast<G4int>(theBaseZ), static_cast<G4int>(theBaseA+1), 0);
...
void DsG410NNDCParticleHPCaptureFS::UpdateNucleus( const G4Fragment* gamma , G4double eGamma )
{
...
//void DsG410NNDCParticleHPCaptureFS::Init (G4double A, G4double Z, G4String & dirName, G4String & )
void DsG410NNDCParticleHPCaptureFS::Init (G4double A, G4double Z, G4int M, G4String & dirName, G4String &, G4ParticleDefinition* )
{
    G4String tString = "/FS/";
    theBaseA = A;
    theBaseZ = Z;
    ...
    /*
    G4bool dbool;
    G4ParticleHPDataUsed aFile = theNames.GetName(theBaseA, 
						 theBaseB, 
